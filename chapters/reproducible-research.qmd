...
The objective of this five-day [training course](https://rdatatoolbox.github.io/), co-organized by the [FRB-CESAB](https://www.fondationbiodiversite.fr/en/about-the-foundation/le-cesab/) and the [RT EcoStat](https://sites.google.com/site/gdrecostat/), is to train researchers in **reproducibility**, **software development** and **version control** tools (e.g. `git`, `GitHub`, `Quarto`, `renv`, `Docker`), applied to biodiversity research.
:::
...
During this training course we will introduce [Docker](https://www.docker.com/get-started/), a containerization solution to reach a high level of reproducibility. But **the installation of Docker is not required** as no exercise is planned for this topic.
:::
...
| :----------- | :---------------------------------------------------------------- | :----------------------------------------------------: |
| `R 4.6.1`    | The R environment                                                 | [link](https://cran.r-project.org/)                    |
...
Please install the following {{< fa brands r-project >}} packages:
...
In order to assist us in creating the structure of our research compendium (project organization), 
we will use the [`usethis`](https://usethis.r-lib.org/) 
package. This package allows for the automation of creating files and directories 
specific to a research compendium and to an {{< fa brands r-project >}} package.


To make full use of the `usethis` package, you will need to store certain information locally 
(first name, last name, email, ORCID, communication protocol with GitHub). 
This information will automatically populate certain files in the compendium. 
You will store this information in the `~/.Rprofile` file.

Run the command in {{< fa brands r-project >}} console: `usethis::edit_r_profile()`. 

This will open the `~/.Rprofile` file. Add the following lines at the end of this file.
...
options(
  usethis.full_name = "Jane Doe",                   ## replace this value
  usethis.description = list(
    "Authors@R" = utils::person(
      given = "Jane",                               ## replace this value
      family = "Doe",                               ## replace this value
      email = "jane.doe@mail.com",                  ## replace this value
      role = c("aut", "cre", "cph"),
      comment = c(ORCID = "0000-0000-0000-0000")    ## replace this value
    )
  ),
...
## $`Authors@R`
## [1] "Jane Doe <jane.doe@mail.com> [aut, cre, cph] (ORCID: <https://orcid.org/0000-0000-0000-0000>)"
```
